...
### Stripe Management
```bash
npm run validate-stripe-prices  # Validate Stripe price IDs
```
...
users/{userId}/
  ...
  └── subscription/
      ├── status                  # Subscription status (read-only for users)
      └── usage                   # Usage tracking (read-only for users)
```
...
**Subscription System** (src/lib/subscription/types.ts):
- Plans: `free | supporter | pro`
- Status: `free | active | canceled | past_due | trialing | incomplete | incomplete_expired | unpaid`
...
- `SubscriptionContext`: Manages subscription status, limits, and usage tracking
...
- Subscription hook: `use-subscription.tsx` integrates with SubscriptionContext

### API Routes

**Stripe Integration** (src/app/api/stripe/):
- `create-checkout/route.ts`: Create Stripe checkout sessions
- `create-portal/route.ts`: Create Stripe customer portal sessions
- `subscription-status/route.ts`: Get subscription status
- `webhook/route.ts`: Handle Stripe webhooks (subscription updates)
- `sync-subscription/route.ts`: Sync subscription data from Stripe
- `plans/route.ts`: Get available subscription plans
...
**Feature Gates**:
- Free plan: Basic journaling, AI conversations with bring-your-own-key
- Supporter plan: Cosmetic perks (badge, accent colors)
- Pro plan: Advanced analysis, multi-entry analysis, period summaries, exports, custom templates, semantic search

**Usage Tracking** (src/lib/subscription/usage-tracker.ts):
- Tracks entries created and model usage per user
- Monthly reset cycle
- Firestore document: `users/{userId}/subscription/usage`
...
- Use `scripts/validate-stripe-prices.ts` to verify configuration
...
- Stripe configuration (secret key, webhook secret, 8 price IDs for both plans/currencies/cycles)
...
- AI features require users to provide their own OpenRouter API key (BYOK model for free tier)
